...
public interface EntityCopier<T extends VulpesModel, E> {
     ...
     * {@code targetKey} (or the source's own key, when {@code null} or blank), including the
     * requested relations.
     *
     ...
     */
    T copy(UUID sourceProjectId, UUID sourceId, @Nullable UUID targetProjectId, @Nullable String targetKey, Set<E> relations);
     ...
     * @return the copied entity
     */
    default T copy(UUID sourceProjectId, UUID sourceId, @Nullable UUID targetProjectId, @Nullable String targetKey) {
        return this.copy(sourceProjectId, sourceId, targetProjectId, targetKey, Set.of());
    }
}
